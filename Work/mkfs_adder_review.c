...
int add_file_to_filesystem(const char* input_path, const char* output_path, const char* file_path) {
    ...
    uint64_t blocks_needed = (file_stat.st_size + BS - 1) / BS; // ceiling division
    ...
    uint8_t* file_data = malloc(file_stat.st_size);
    if (!file_data) {
    ...
    if (fread(file_data, 1, file_stat.st_size, file_to_add) != (size_t)file_stat.st_size) {
    ...
    for (uint32_t i = 0; i < blocks_needed; i++) {
        ...
        if (file_offset + BS > (uint64_t)file_stat.st_size) {
            data_to_write = file_stat.st_size - file_offset;
        }
